...
let imageClass1 = cardObject1.imageClass;
let imageClass2 = cardObject2.imageClass;
...
/*function setUpGame() {
  let cardClasses = shuffleCardImageClasses();
  let cards = createCards(document.getElementById('card-container'), cardClasses);
  cards.forEach(card => {
      if (!card.element.classList.contains('flipped')) {
        card.element.classList.add('flipped');
        card.element.addEventListener('click', () => onCardFlipped(card));
      }
    });
}*/
